...
// Setup Token If Not Exists
if (!token) {
  const newToken = prompt("Enter your token", null);
  if (newToken) {
      window.localStorage.setItem("x-auth-token", newToken);
  }
  location.reload();
}
...
window.StoreinoApp.$store = {
  ...
  get: async function (module, params) {
      let response = await http.get(`/api/${module}/get`, { params });
      return response.data;
  }
};
...
(async () => {
  ...
  // Return Translate
  window.StoreinoApp.filter("t", (val) => {
      return val;
  });

  // Create XMLHttpRequest For Get Translate
  ...
  xhttp.onreadystatechange = function () {
      ...
      window.StoreinoApp.filter("t", (val) => {
          ...
          if (messages[lang] && messages[lang][val]) return messages[lang][val];
          if (messages["EN"] && messages["EN"][val]) return messages["EN"][val];
          return val;
      });
      }
  };
  xhttp.open("GET", "./translate.json", true);
  xhttp.send();
  ...
  // Create Submit Bar
  let submitBar = document.createElement("div");
  submitBar.classList.add("sticky-submit-bar");
